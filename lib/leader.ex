...
def start config do
  ...
  receive do
    { :bind, acceptors, replicas, leaders, monitor } ->
      spawn Scout, :start, [self(), propose_num, acceptors, config, monitor]
      next Map.new, false, acceptors, replicas, propose_num, config, false, leaders, monitor
  end
end

def next proposals, active, acceptors, replicas, propose_num, config, sleep_random, leaders, monitor do
  receive do
    { :sleep_random } ->
      next proposals, active, acceptors, replicas, propose_num, config, true, leaders, monitor

    { :propose, s, c } ->
      proposals = if not Map.has_key?(proposals, s) do
        if active do
          spawn Commander, :start, [self(), acceptors, replicas, {propose_num, s, c}, config, monitor]
        end
        Map.put(proposals, s, c)
      else
        proposals
      end
      next proposals, active, acceptors, replicas, propose_num, config, sleep_random, leaders, monitor
    { :adopted, acc_p, pvals } ->
      proposals = Map.merge(proposals, Map.new(p_max pvals))
      for {s, c} <- proposals do
        spawn Commander, :start, [self(), acceptors, replicas, {acc_p, s, c}, config, monitor]
      end
      next proposals, true, acceptors, replicas, propose_num, config, sleep_random, leaders, monitor
    ...
    { :preempted, {r, l} } ->
      ...
      if {r, l} > propose_num do
        if config.livelock_prevention do
        ...
        spawn Scout, :start, [self(), propose_num, acceptors, config, monitor]
        next proposals, false, acceptors, replicas, propose_num, config, sleep_random, leaders, monitor
      else
        next proposals, active, acceptors, replicas, propose_num, config, sleep_random, leaders, monitor
      end
